...
wss.on('connection', ws => {
  ws.on('message', raw => {
    ...
    if(m.t==='settings'){
      ...
      T.players.forEach(p=>{
        p.stack=T.buyIn; p.buyIns=1; p.sessionIn=T.buyIn;
        const pr=profiles[p.phone];
        if(pr){
    ...
    if(m.t==='reset'){
      ...
      T.players.forEach(p=>{ p.stack=T.buyIn; p.buyIns=1; p.sessionIn=T.buyIn; p.in=false; p.folded=false; p.allIn=false;
