...
const RUNOUT_MS = parseInt(process.env.RUNOUT_MS || '5000', 10);      // suspense between cards
const AUTO_DEAL_MS = parseInt(process.env.AUTO_DEAL_MS || '10000', 10); // pause between hands
let runoutTimer = null, autoDealTimer = null;
const T = {
  ...
  turnEndsAt:0, runningOut:false, allInReveal:false, nextDealAt:0
};
...
/* record a buy-in (initial seat, rebuy, or returning with a banked stack) */
function recordBuyIn(pr, amount){
  const amt = (amount && amount>0) ? amount : T.buyIn;
  pr.chipsIn += amt;
  pr.openChipsIn += amt;
  const c = cashPerBuyIn() * (T.buyIn>0 ? amt/T.buyIn : 0);
  pr.cashIn += c;
  pr.openCashIn += c;
  pr.buyInsTotal += 1;
  saveLedger();
...
function cashOut(p){
  ...
  pr.lastStack = p.stack;        // what they can choose to return with
 ...
 * The ledger keeps lifetime results; this keeps the table itself — stacks,
 * the hand in progress, whose turn it is — so a redeploy or crash resumes
 * instead of wiping the night. Requires DATA_DIR on a Railway Volume.
 */
const TABLE_FILE = path.join(DATA_DIR, 'table.json');
const TABLE_FIELDS = ['host','buyIn','sb','bb','cashPerBuyIn','phase','players','dealer',
  'handNum','deck','community','currentBet','minRaise','turn','acted','result','lastHand',
  'runningOut','allInReveal'];

function tableSnapshot(){
  const o={};
  TABLE_FIELDS.forEach(k=>{ o[k]=T[k]; });
  o.kicked=[...kickedNames];
  o.savedAt=Date.now();
  return o;
}
let tableSaveQueued=false;
function saveTable(){
  if(tableSaveQueued) return;
  tableSaveQueued=true;
  setTimeout(()=>{ tableSaveQueued=false; writeTableNow(); }, 250);
}
function writeTableNow(){
  try{
    fs.mkdirSync(DATA_DIR,{recursive:true});
    const tmp=TABLE_FILE+'.tmp';
    fs.writeFileSync(tmp, JSON.stringify(tableSnapshot()));
    fs.renameSync(tmp, TABLE_FILE);
  }catch(e){ console.error('Table save failed: '+e.message); }
}
function loadTable(){
  try{
    if(!fs.existsSync(TABLE_FILE)){ console.log('No saved table; starting fresh.'); return; }
    const o=JSON.parse(fs.readFileSync(TABLE_FILE,'utf8'));
    if(!o || !Array.isArray(o.players)){ console.log('Saved table unreadable; starting fresh.'); return; }
    TABLE_FIELDS.forEach(k=>{ if(o[k]!==undefined) T[k]=o[k]; });
    (o.kicked||[]).forEach(k=>kickedNames.add(k));
    // runtime-only bits never carry across a restart
    T.turnEndsAt=0; T.nextDealAt=0;
    T.players.forEach(p=>{ p.connected=false; p.auto=null; });
    const mins=o.savedAt?Math.round((Date.now()-o.savedAt)/60000):'?';
    console.log(`Table restored: ${T.players.length} player(s), hand #${T.handNum}, phase ${T.phase} (saved ${mins} min ago)`);
  }catch(e){ console.error('Table load failed: '+e.message); }
}
/* after a restart, pick up anything that was mid-flight */
function resumeAfterLoad(){
  if(T.runningOut && T.community.length<5){ stepRunout(); }
  else if(T.runningOut){ T.runningOut=false; }
  maybeScheduleAutoDeal();
}
...
function newPlayer(phone,name,startStack){
  const st = (startStack && startStack>0) ? startStack : T.buyIn;
  return { id:phone, phone, name, stack:st, buyIns:1, sessionIn:st, auto:null,
           in:false, folded:false, allIn:false,
           bet:0, committed:0, cards:[], needsRebuy:false, connected:true, banked:0 };
}
...
function startHand(){
  ...
  clearTimeout(autoDealTimer); autoDealTimer=null;
  T.runningOut=false; T.allInReveal=false; T.nextDealAt=0;
  clearAutos();
...
function advancePhase(){
  ...
  if(T.phase==='river'){ showdown(); return; }
  const actors=T.players.filter(canAct);
  if(actors.length<=1){ runOut(); return; }   // betting is closed — deal the rest slowly, one at a time
...
/* When betting is closed but cards are still to come (someone's all-in), flip
 * everyone's hole cards face up and deal the rest one at a time, slowly. */
function runOut(){
  clearTimeout(turnTimer); turnTimer=null; armedKey=''; T.turnEndsAt=0;
  if(T.community.length>=5){ showdown(); return; }
  T.runningOut = true;
  T.allInReveal = true;      // viewFor will expose live players' hole cards
  T.turn = -1;
  broadcast();               // beat one: cards flip up, nothing dealt yet
  stepRunout();
}
function stepRunout(){
  clearTimeout(runoutTimer);
  runoutTimer = setTimeout(()=>{
    if(!T.runningOut) return;
    if(T.community.length < 5){
      T.community.push(T.deck.pop());
      T.phase = T.community.length<=3 ? 'flop' : (T.community.length===4 ? 'turn' : 'river');
      broadcast();
      stepRunout();
    } else {
      T.runningOut = false;
...
/* auto-deal the next hand so nobody has to press a button */
function maybeScheduleAutoDeal(){
  if(T.phase!=='result'){ clearTimeout(autoDealTimer); autoDealTimer=null; T.nextDealAt=0; return; }
  if(autoDealTimer) return;                                  // already counting down
  if(T.players.filter(p=>p.stack>0).length < 2){ T.nextDealAt=0; return; }
  T.nextDealAt = Date.now() + AUTO_DEAL_MS;
  autoDealTimer = setTimeout(()=>{
    autoDealTimer = null; T.nextDealAt = 0;
    if(T.phase==='result' && T.players.filter(p=>p.stack>0).length>=2){
      if(startHand()) broadcast();
    }
  }, AUTO_DEAL_MS);
}
function showdown(){
...
function checkBusted(){
  ...
  if(T.result){
    T.lastHand = { handNum:T.handNum, lines:T.result.lines,
                   reveals:T.result.reveals, community:T.community.slice() };
  }
...
function viewFor(pid){
  const showdown = T.phase==='result' || T.allInReveal;
  return {
    ...
    lastHand: T.lastHand || null,
    runningOut: !!T.runningOut,
    allInReveal: !!T.allInReveal,
    nextDealMs: T.nextDealAt ? Math.max(0, T.nextDealAt - Date.now()) : 0,
    community:T.community,
    pot:T.players.reduce((t,p)=>t+p.committed,0),
    result:T.result,
    you:pid,
    yourAuto: (T.players.find(p=>p.id===pid)||{}).auto || null,
    players:T.players.map(p=>({
      ...
      sessionIn: p.sessionIn || (p.buyIns*T.buyIn),
      banked: p.banked || 0,
...
/* pre-set actions: fire the instant it becomes that player's turn */
function processAuto(){
  for(let guard=0; guard<12; guard++){
    if(T.runningOut) return;
    if(!['preflop','flop','turn','river'].includes(T.phase)) return;
    const p = T.players[T.turn];
    if(!p || !p.auto || !canAct(p)) return;
    const facingBet = p.bet !== T.currentBet;
    if(!facingBet){
      applyAction(p.id, 'check', 0);          // free to see the next card — take it
    } else if(p.auto === 'checkfold'){
      p.auto = null;
      applyAction(p.id, 'fold', 0);
    } else {
      p.auto = null;                           // "check only" but there's a bet: hand it back to them
      return;
    }
  }
}
function clearAutos(){ T.players.forEach(p=>{ p.auto=null; }); }
...
wss.on('connection', ws => {
  ws.on('message', raw => {
    ...
    if(m.t==='sit'){
      ...
      // returning after a cash-out: let them pick a fresh buy-in or their banked stack
      if(!p && !(pr.openStack>0 && pr.openChipsIn>0)
         && pr.lastStack>0 && pr.lastStack!==T.buyIn && !m.buyInChoice){
        ws.send(JSON.stringify({t:'choosebuyin',
          standard:T.buyIn, banked:pr.lastStack, name, phone}));
        return;
      }
      ...
      if(!p){
        ...
        if(pr.openStack>0 && pr.openChipsIn>0){
          ...
          p=newPlayer(phone,name,pr.openStack); T.players.push(p);
          p.sessionIn=pr.openChipsIn;
          p.buyIns=Math.max(1, Math.round(pr.openChipsIn/Math.max(1,T.buyIn)));
        } else {
          const useBanked = m.buyInChoice==='banked' && pr.lastStack>0;
          const amt = useBanked ? pr.lastStack : T.buyIn;
          p=newPlayer(phone,name,amt); T.players.push(p);
          pr.sessions+=1;
          recordBuyIn(pr, amt);
          p.sessionIn=amt;
          pr.openStack=p.stack;
          pr.lastStack=0;             // banked stack has been brought back to the table
        }
        saveLedger();
    ...
    if(m.t==='auto'){
      ...
      if(!p) return;
      const mode=m.mode;
      p.auto = (mode==='check'||mode==='checkfold') ? mode : null;
      broadcast(); return;
    }
    if(m.t==='action'){
      if(T.phase!=='lobby'&&T.phase!=='result'&&!T.runningOut){
        const me2=T.players.find(x=>x.id===pid);
        if(me2) me2.auto=null;                 // acting by hand overrides any pre-set
        applyAction(pid,m.action,m.amount|0);
        broadcast();
      }
      return;
    }
    if(m.t==='splash'){
      ...
      if(!p) return;
      const now=Date.now();
      if(p.lastSplashAt && now-p.lastSplashAt < 2500) return;   // cooldown on the nonsense
      p.lastSplashAt=now;
      const payload=JSON.stringify({t:'splash', id:p.id, from:p.name});
    ...
    if(m.t==='rebuy'){
      ...
      if(p&&p.needsRebuy){
        ...
        p.sessionIn=(p.sessionIn||0)+T.buyIn;
    ...
    if(m.t==='rename'){
      ...
      if(pr){ pr.name=name; saveLedger(); }   // ledger follows the phone, not the name
      broadcast(); return;
    }
    if(m.t==='bank'){
      // take profit off the table: reset to the standard buy-in, bank the rest
      if(T.phase!=='lobby' && T.phase!=='result'){
        ws.send(JSON.stringify({t:'err',msg:'You can only bank profit between hands.'})); return;
      }
      const p=T.players.find(x=>x.id===pid);
      if(!p) return;
      const diff=p.stack-T.buyIn;
      if(diff<=0){ ws.send(JSON.stringify({t:'err',msg:'Nothing to bank — you need more than the buy-in.'})); return; }
      const pr=profiles[p.phone];
      if(pr){
        pr.chipsOut += diff;                                     // settled into all-time
        pr.cashOut  += (T.buyIn>0 ? diff/T.buyIn : 0) * cashPerBuyIn();
      }
      p.stack = T.buyIn;
      p.banked = (p.banked||0) + diff;                           // keeps tonight's net honest
      if(pr){ pr.openStack = p.stack; saveLedger(); }
      ws.send(JSON.stringify({t:'banked', amount:diff}));
      broadcast(); return;
    }
    if(m.t==='mystats'){
      const pr=profiles[pid];
    ...
    if(m.t==='settings'){
      ...
      T.players.forEach(p=>{
        p.stack=T.buyIn; p.buyIns=1; p.sessionIn=T.buyIn; p.banked=0;
        const pr=profiles[p.phone];
        if(pr){
    ...
    if(m.t==='reset'){
      ...
      T.phase='lobby'; T.handNum=0; T.dealer=undefined; T.community=[]; T.deck=[]; T.result=null; T.lastHand=null;
      T.players.forEach(p=>{ p.stack=T.buyIn; p.buyIns=1; p.sessionIn=T.buyIn; p.banked=0; p.in=false; p.folded=false; p.allIn=false;
...
if(!process.env.DATA_DIR){
  console.warn('WARNING: DATA_DIR is not set, so the ledger and table live in the container and');
  console.warn('         WILL BE WIPED on the next deploy. Attach a Railway Volume and set');
  console.warn('         DATA_DIR to its mount path (e.g. /data) to keep them.');
}

/* flush to disk before the container goes away on a redeploy */
let shuttingDown=false;
function shutdown(sig){
  if(shuttingDown) return;
  shuttingDown=true;
  console.log(sig+' received — saving table and ledger before exit.');
  try{ writeTableNow(); }catch(e){}
  try{
    fs.mkdirSync(DATA_DIR,{recursive:true});
    fs.writeFileSync(LEDGER_FILE, JSON.stringify(profiles,null,2));
  }catch(e){ console.error('Final ledger save failed: '+e.message); }
